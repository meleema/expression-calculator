...
1. x + y               → запросит значения x и y
2. a * b + c           → запросит значения a, b, c

# Обработка ошибок

1. 2 + * 3             → Ошибка: Неожиданный токен (позиция: 3)
2. 1 / 0               → Ошибка: Деление на ноль (позиция: 2)
3. sqrt(-1)            → Ошибка: Корень из отрицательного числа (позиция: 0)
4. unknown * 2         → Ошибка: Неизвестная переменная (позиция: 0)
